...
export const get = query({
    ...
    handler: async(ctx, args)=>{
        ...
        if(!currentUser){
            throw new ConvexError("User can't be found")
        }
        ...
        if(!conversation.isGroup){
            ...
            const otherMembers = (await Promise.all(allConversationMembership.filter(membership => membership.memberId !== currentUser._id))).map(async membership => {
